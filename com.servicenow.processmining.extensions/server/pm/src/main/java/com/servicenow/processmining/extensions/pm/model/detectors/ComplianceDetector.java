...
public class ComplianceDetector
{
        ...
        if (getModel().getFilters().size() > 0) {
            for (ProcessMiningModelFilter filter : getModel().getFilters().values()) {
                if (filter.getFilterTransitions() != null) {
                    ProcessMiningModelVariant v = getReferencePathVariation(filter);
    ...
    private ProcessMiningModelVariant getReferencePathVariation(final ProcessMiningModelFilter filter)
    {
        ProcessMiningModelFilterTransitions filterTransitions = filter.getFilterTransitions();
        ProcessMiningModelVariant pmmv = new ProcessMiningModelVariant("reference_path");
    ...
    {
        for (String referencePathVariant : getReferencePathGraphs().keySet()) {
            Graph<String, DefaultEdge> referencePathGraph = getReferencePathGraphs().get(referencePathVariant);
    ...
    private boolean checkVariantAgainstReferencePath(final ProcessMiningModelVariant variant, final ProcessMiningModelVariant referencePathVariant, final Graph<String, DefaultEdge> referencePathGraph)
    {
        ...
        for (String node : variant.getPath()) {
            ...
            if (processedFirstNode) {
                ...
                if (!referencePathGraph.containsVertex(previousNode)) {
                    variant.addNonComplianceDetails("The path: " + getModel().getVariantLabeledPath(variant) + " is NOT compliant with the reference path.");
                    variant.addNonComplianceDetails("Reference Path: (" + getModel().getVariantLabeledPath(referencePathVariant) + ").");
                    variant.addNonComplianceDetails("The node: (" + getModel().getNodes().get(previousNode).getName() + ") is not in the reference path.");
                    isCompliant = false;
                    break;
                }
                    ...
                    variant.addNonComplianceDetails("The path: " + getModel().getVariantLabeledPath(variant) + " is NOT compliant with the reference path.");
                    variant.addNonComplianceDetails("Reference Path: (" + getModel().getVariantLabeledPath(referencePathVariant) + ").");
                    variant.addNonComplianceDetails("The node: (" + getModel().getNodes().get(node).getName() + ") is not in the reference path.");
                    isCompliant = false;
                    break;
                }
                ...
                // or through a different path...
                    ...
                    if (!variant.getPath().subList(0, nodeTraversalPosition).contains(node)) {
                        variant.addNonComplianceDetails("The path: " + getModel().getVariantLabeledPath(variant) + " is NOT compliant with the reference path.");
                        variant.addNonComplianceDetails("Reference Path: (" + getModel().getVariantLabeledPath(referencePathVariant) + ").");
                        variant.addNonComplianceDetails("There is no transition from (" + getModel().getNodes().get(previousNode).getName() + ") to (" + getModel().getNodes().get(node).getName() + ") in the reference path.");
                        isCompliant = false;
                        break;    
                    }
                }
        ...
        if (isCompliant) {
            variant.addNonComplianceDetails("The path: " + getModel().getVariantLabeledPath(variant) + " is compliant with the reference path.");
            variant.addNonComplianceDetails("Reference Path: (" + getModel().getVariantLabeledPath(referencePathVariant) + ").");
        }
