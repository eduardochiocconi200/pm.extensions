...
public class ProcessMiningVariantRetrievalZurich
    extends ProcessMiningModelRetrieval
{
    private String entityId = null;
    private int numberOfVariants = 0;

    public ProcessMiningVariantRetrievalZurich(final ServiceNowInstance instance, final String modelVersionId, final String entityId, final int numberOfVariants)
    {
    ...
    {
        logger.debug("Enter ProcessMiningVariantRetrievalZurich.getEmptyFilterPayload()");
        ...
        logger.debug("Exit ProcessMiningVariantRetrievalZurich.getEmptyFilterPayload()");
        return payload;
    }

    protected String getFilterPayload(final String entityId, final ProcessMiningModelFilter filter)
    {
        logger.debug("Enter ProcessMiningVariantRetrievalZurich.getFilterPayload()");
        String variables = "{\n" + 
                "\t\"versionId\": \"" + getProcessModelVersionId() + "\",\n" + 
                "\t\"entityId\": \"" + getEntityId() + "\",\n" + 
                "\t\"filterSets\": {\n" + 
        ...
        logger.debug("payload: (" + payload + ")");
        logger.debug("Exit ProcessMiningVariantRetrievalZurich.getFilterPayload()");
        return payload;
    }
    ...
    private final static String query = "query snProminWorkbenchConnected($versionId:ID!$filterSets:GlidePromin_FilterInput$variantsLimit:Int$variantsOffset:Int$variantsOrderBy:GlidePromin_VariantSortField$variantsOrderByDesc:Boolean$variantsQuery:String$entityId:ID){GlidePromin_Query{variantResult(versionId:$versionId filterSets:$filterSets entityId:$entityId offset:$variantsOffset limit:$variantsLimit orderBy:$variantsOrderBy orderByDesc:$variantsOrderByDesc query:$variantsQuery){totalVariants entityId variants{...FRAGMENT_VARIANTS}}}}fragment FRAGMENT_VARIANTS on GlidePromin_Variant{id statsId entityId nodes caseIds nodeCount frequency totalDuration maxDuration minDuration avgDuration trimmedAverage stdDeviation medianDuration healthScore totalTouchPoints avgTouchPoints totalIdleTime avgIdleTime}";
