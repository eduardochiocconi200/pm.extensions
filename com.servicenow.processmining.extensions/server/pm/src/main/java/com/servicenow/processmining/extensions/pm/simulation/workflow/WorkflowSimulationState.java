...
public class WorkflowSimulationState
{
    ...
    private HashMap<String, Integer> maxActivityStateCount = null;
    private HashMap<String, Vector<WorkflowInstanceQueuedInfo>> activityStateQueue = null;
    private HashMap<String, Integer> maxActivityStateQueue = null;
    ...
    public WorkflowSimulationState(final ProcessMiningModel processModel, final WorkflowSimulator simulator)
    {
    ...
    {
        return this.activityStateCount;
    }

    public HashMap<String, Integer> getMaxActivityStateCount()
    {
        return this.maxActivityStateCount;
    }

    public HashMap<String, Integer> getMaxActivityStateQueue()
    {
        return this.maxActivityStateQueue;
    }

    public void incrementCount(final String instanceId, final String currentActivityState)
    {
        ...
        logger.debug("[" + instanceId + "] - Incrementing Resource Usage: (" + currentActivityState + "). From: (" + currentCount + ") to (" + (currentCount+1) + ")");

        int activityResourceCapacity = getProcessModel().getNodes().get(currentActivityState).getResources().getCapacity();
        // If there is still resource capacity...
        if (activityResourceCapacity == -1 || currentCount+1 <= activityResourceCapacity) {
            logger.debug("[" + instanceId + "] - New Max for (" + currentActivityState + "). Setting: (" + (currentCount+1) + ")");
            this.getMaxActivityStateCount().remove(currentActivityState);
            this.getMaxActivityStateCount().put(currentActivityState, currentCount+1);
        }
        // If we have reached resource capacity and we need to enqueue...
        else {
            int newQueueSize = getMaxActivityStateQueue().get(currentActivityState) == null ? 1 : (getMaxActivityStateQueue().get(currentActivityState) + 1);
            this.getMaxActivityStateQueue().remove(currentActivityState);
            this.getMaxActivityStateQueue().put(currentActivityState, newQueueSize);
        }
    }

    public void decrementCount(final String instanceId, final String currentActivityState)
    {
        int currentCount = getActivityStateCount().get(currentActivityState).intValue();
        getActivityStateCount().remove(currentActivityState);
        getActivityStateCount().put(currentActivityState, currentCount - 1);
        logger.debug("[" + instanceId + "] - Decrementing Resource Usage: (" + currentActivityState + "). From: (" + currentCount + ") to (" + (currentCount-1) + ")");
    }

    public void increaseUsage(final String instanceId, final String currentNode)
    {
        activityStateQueueResource.get(currentNode).increaseUsage();
    }

    public void decreaseUsage(final String instanceId, final String currentNode)
    {
    ...
    private void initializeWorkflowMetadata()
    {
        ...
        maxActivityStateCount = new HashMap<String, Integer>();
        activityStateQueue = new HashMap<String, Vector<WorkflowInstanceQueuedInfo>>();
        maxActivityStateQueue = new HashMap<String, Integer>();
        ...
        for (ProcessMiningModelNode node : getProcessModel().getNodes().values()) {
            ...
            if (node.getResources() != null) {
                ...
                maxActivityStateCount.put(node.getId(), 0);
                activityStateQueue.put(node.getId(), new Vector<WorkflowInstanceQueuedInfo>());
                maxActivityStateQueue.put(node.getId(), 0);
    ...
    public void checkIfThereAreEnquedWorkflowInstances(final String instanceId, final String executedNode)
    {
        ...
        if (executedNode != null && !executedNode.equals("") && activityStateQueueResource.get(executedNode).hasCapacity()) {
            ...
            if (activityStateQueue.get(executedNode).size() > 0) {
                ...
                increaseUsage(instanceId, enqueuedWI.getFromNode());
    ...
    private static final Logger logger = LoggerFactory.getLogger(WorkflowSimulationState.class);
}