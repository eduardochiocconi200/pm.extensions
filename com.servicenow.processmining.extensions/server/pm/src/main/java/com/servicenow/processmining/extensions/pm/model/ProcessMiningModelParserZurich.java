...
public class ProcessMiningModelParserZurich
    extends ProcessMiningModelParser
{
    public ProcessMiningModelParserZurich(final String versionId)
    {
        super(versionId);
    }
        ...
        if (processModelJSON.has("data")) {
            ...
            if (dataObj.has("GlidePromin_Query")) {
                ...
                if (queryObj.has("scheduleModel")) {
                    ...
                    if (scheduleModelObj.has("version")) {
                        JSONObject versionObj = (JSONObject) scheduleModelObj.getJSONObject("version");
                        // Project Details ...
                        if (versionObj.has("projectDefinition")) {
                            JSONObject projectDefinitionObj = (JSONObject) versionObj.getJSONObject("projectDefinition");
                            if (projectDefinitionObj != null) {
                                String name = projectDefinitionObj.getString("name");
                                String projectId = projectDefinitionObj.getString("projectId");
                                pmm.setName(name);
                                pmm.setProjectId(projectId);
                            }

                            // Project Entities Details ...
                            if (projectDefinitionObj.has("projectEntities")) {
                                JSONArray projectEntities = (JSONArray) projectDefinitionObj.getJSONArray("projectEntities");
    ...
    {
        JSONObject processModelJSONbackup = processModelJSON;
        processModelJSON = new JSONObject(processModelVariantsJSONString);
        parseVariations();
        processModelJSON = processModelJSONbackup;
    ...
    protected void parseVariations()
    {
        if (processModelJSON.has("data")) {
            ...
            if (dataObj.has("GlidePromin_Query")) {
                JSONObject queryObj = (JSONObject) dataObj.getJSONObject("GlidePromin_Query");
                if (queryObj.has("variantResult")) {
                    parseVariantResult(queryObj);
                }
                else if (queryObj.has("scheduleModel")) {
                    JSONObject scheduleModelObj = (JSONObject) queryObj.getJSONObject("scheduleModel");
